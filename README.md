...
- 🎯 Entry points to execute your program using `python -m <base>` or `$ base` with basic CLI argument parsing.
...
Awesome base created by isaiah2004
...
```bash
git clone https://github.com/isaiah2004/base base
cd base
make install
```
...
```bash
base create-db   # run once
base populate-db  # run once (optional)
base add-user -u admin -p 1234  # ads a user
base run
```
